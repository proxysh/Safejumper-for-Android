...
public class OpenVPNTileService extends TileService implements VpnStatus.StateListener {
    ...
    public void onClick() {
        ...
        if (bootProfile == null) {
            Toast.makeText(this, com.proxy.sh.shieldtra.R.string.novpn_selected, Toast.LENGTH_SHORT).show();
        } else {
            if (!isLocked())
    ...
    public void updateState(String state, String logmessage, int localizedResId, ConnectionStatus level) {
        ...
        if (level == ConnectionStatus.LEVEL_AUTH_FAILED || level == ConnectionStatus.LEVEL_NOTCONNECTED) {
            ...
            vpn = getQSVPN();
            if (vpn == null) {
                t.setLabel(getString(com.proxy.sh.shieldtra.R.string.novpn_selected));
                t.setState(Tile.STATE_UNAVAILABLE);
            } else {
                t.setLabel(getString(com.proxy.sh.shieldtra.R.string.qs_connect, vpn.getName()));
            ...
            if (vpn == null)
                ...
                name = vpn.getName();
            t.setLabel(getString(com.proxy.sh.shieldtra.R.string.qs_disconnect, name));
