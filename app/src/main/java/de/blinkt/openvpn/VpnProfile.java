            ...
            case VpnProfile.TYPE_EXTERNAL_APP:
                if (!configForOvpn3) {
                    ...
                    if (ks != null) {
                        ...
                        cfg.append(context.getString(com.proxy.sh.shieldtra.R.string.keychain_access)).append("\n");
                        if (Build.VERSION.SDK_INT == Build.VERSION_CODES.JELLY_BEAN)
                            if (!mAlias.matches("^[a-zA-Z0-9]$"))
                                cfg.append(context.getString(com.proxy.sh.shieldtra.R.string.jelly_keystore_alphanumeric_bug)).append("\n");
                    }
                }
                break;
...
//        }
    ...
    synchronized String[] getExternalCertificates(Context context, int tries) {
        ...
        try {
            ...
            if (caChain.length <= 1 && TextUtils.isEmpty(mCaFilename)) {
                VpnStatus.logMessage(VpnStatus.LogLevel.ERROR, "", context.getString(com.proxy.sh.shieldtra.R.string.keychain_nocacert));
            } else {
            ...
            e.printStackTrace();
            VpnStatus.logError(com.proxy.sh.shieldtra.R.string.keyChainAccessError, e.getLocalizedMessage());

            VpnStatus.logError(com.proxy.sh.shieldtra.R.string.keychain_access);
            if (Build.VERSION.SDK_INT == Build.VERSION_CODES.JELLY_BEAN) {
                if (!mAlias.matches("^[a-zA-Z0-9]$")) {
                    VpnStatus.logError(com.proxy.sh.shieldtra.R.string.jelly_keystore_alphanumeric_bug);
                }
            }
            return null;
    ...
    public int checkProfile(Context context, boolean useOpenVPN3) {
        if (mAuthenticationType == TYPE_KEYSTORE || mAuthenticationType == TYPE_USERPASS_KEYSTORE || mAuthenticationType == TYPE_EXTERNAL_APP) {
            if (mAlias == null)
                return com.proxy.sh.shieldtra.R.string.no_keystore_cert_selected;
        } else if (mAuthenticationType == TYPE_CERTIFICATES || mAuthenticationType == TYPE_USERPASS_CERTIFICATES) {
            if (TextUtils.isEmpty(mCaFilename))
                return com.proxy.sh.shieldtra.R.string.no_ca_cert_selected;
        }

        if (mCheckRemoteCN && mX509AuthType == X509_VERIFY_TLSREMOTE)
            return com.proxy.sh.shieldtra.R.string.deprecated_tls_remote;

        if (!mUsePull || mAuthenticationType == TYPE_STATICKEYS) {
            if (mIPv4Address == null || cidrToIPAndNetmask(mIPv4Address) == null)
                return com.proxy.sh.shieldtra.R.string.ipv4_format_error;
        }
        if (!mUseDefaultRoute) {
            if (!TextUtils.isEmpty(mCustomRoutes) && getCustomRoutes(mCustomRoutes).size() == 0)
                return com.proxy.sh.shieldtra.R.string.custom_route_format_error;
        ...
        if (mUseTLSAuth && TextUtils.isEmpty(mTLSAuthFilename))
            return com.proxy.sh.shieldtra.R.string.missing_tlsauth;
        ...
        if ((mAuthenticationType == TYPE_USERPASS_CERTIFICATES || mAuthenticationType == TYPE_CERTIFICATES)
            ...
            return com.proxy.sh.shieldtra.R.string.missing_certificates;

        if ((mAuthenticationType == TYPE_CERTIFICATES || mAuthenticationType == TYPE_USERPASS_CERTIFICATES)
                && TextUtils.isEmpty(mCaFilename))
            return com.proxy.sh.shieldtra.R.string.missing_ca_certificate;
        ...
        if (noRemoteEnabled)
            return com.proxy.sh.shieldtra.R.string.remote_no_server_selected;

        if (useOpenVPN3) {
            if (mAuthenticationType == TYPE_STATICKEYS) {
                return com.proxy.sh.shieldtra.R.string.openvpn3_nostatickeys;
            }
            if (mAuthenticationType == TYPE_PKCS12 || mAuthenticationType == TYPE_USERPASS_PKCS12) {
                return com.proxy.sh.shieldtra.R.string.openvpn3_pkcs12;
            }
            for (Connection conn : mConnections) {
                if (conn.mProxyType == Connection.ProxyType.ORBOT || conn.mProxyType == Connection.ProxyType.SOCKS5)
                    return com.proxy.sh.shieldtra.R.string.openvpn3_socksproxy;
            }
        }
        for (Connection c : mConnections) {
            if (c.mProxyType == Connection.ProxyType.ORBOT) {
                if (usesExtraProxyOptions())
                    return com.proxy.sh.shieldtra.R.string.error_orbot_and_proxy_options;
                if (!OrbotHelper.checkTorReceier(context))
                    return com.proxy.sh.shieldtra.R.string.no_orbotfound;
            }
        }


        // Everything okay
        return com.proxy.sh.shieldtra.R.string.no_error_found;
    ...
    public int needUserPWInput(String transientCertOrPkcs12PW, String mTransientAuthPW) {
        if ((mAuthenticationType == TYPE_PKCS12 || mAuthenticationType == TYPE_USERPASS_PKCS12) &&
            ...
            if (transientCertOrPkcs12PW == null)
                return com.proxy.sh.shieldtra.R.string.pkcs12_file_encryption_key;
        }

        if (mAuthenticationType == TYPE_CERTIFICATES || mAuthenticationType == TYPE_USERPASS_CERTIFICATES) {
            if (requireTLSKeyPassword() && TextUtils.isEmpty(mKeyPassword))
                if (transientCertOrPkcs12PW == null) {
                    return com.proxy.sh.shieldtra.R.string.private_key_password;
                }
        }
        ...
        if (isUserPWAuth() &&
            ...
            return com.proxy.sh.shieldtra.R.string.password;
        }
        return 0;
    }
    ...
    private byte[] getExtAppSignedData(Context c, byte[] data) {
        ...
        try {
            ...
            VpnStatus.logError(com.proxy.sh.shieldtra.R.string.error_extapp_sign, mExternalAuthenticator, e.getClass().toString(), e.getLocalizedMessage());
            return null;
        }
    }
    ...
    private byte[] getKeyChainSignedData(byte[] data) {
        ...
        try {
            ...
            VpnStatus.logError(com.proxy.sh.shieldtra.R.string.error_rsa_sign, e.getClass().toString(), e.getLocalizedMessage());
            return null;
        }
    }
